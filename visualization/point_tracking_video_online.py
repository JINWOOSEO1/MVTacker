...
def main():
    ...
    def render_image(rgb, depth, intr, extr):
        ...
        intr_tensor = torch.from_numpy(intr).to(device)
        extr_tensor = torch.from_numpy(extr).to(device)
        query_points_tensor = torch.from_numpy(query_points).to(device).float()
        ...
        with torch.no_grad():
            ...
            results = mvtracker(
                rgbs=rgb_tensor[None] / 255.0,   
                depths=depth_tensor[None],       
                intrs=intr_tensor[None],         
        ...
        def add_point_cloud(t):
            ...
            for cam_idx in range(B):
                pcd = lift_pixels_to_world(depth_tensor[cam_idx, t, 0], intr_tensor[cam_idx, t], extr_tensor[cam_idx, t])
                pcd_world.append(pcd.detach().cpu().numpy())
                color = rgb_tensor[cam_idx, t] # (3, H, W)
